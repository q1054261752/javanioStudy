...
public class BufferDemo06 {
	...
	// 调用 flip方法, 读取数据  position > limit 将报错
...
//						byteBuffer.put((byte) 5);   //如果写入的数据大于buffer的大小，将报错
		...
		System.out.println("position位置:" + byteBuffer.position());  // position = limit ,读取正常
		System.out.println("limit限制：" + byteBuffer.limit());  // limit 的大小等于 之前 write的数据大小
		...
		System.out.println("position位置:" + byteBuffer.position());   // position > limit ,读取将发生异常
		System.out.println("limit限制：" + byteBuffer.limit());  // limit 的大小等于 之前 write的数据大小
